  ...
  useEffect(() => {
    const fetchSchool = async () => {
      const { data } = await supabase
        .from('schools')
        .select('name, school_address, school_contact_email, slug, primary_color, policy_terms')
        .eq('slug', params.slug)
        .single()
      setSchool(data)
      setLoading(false)
    ...
    <div style={{ minHeight: '100vh', background: '#0f172a', color: '#fff', fontFamily: "'DM Sans', sans-serif", padding: '48px 24px' }}>
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        <div style={{ marginBottom: '40px' }}>
          ...
          <h1 style={{ fontSize: '30px', fontWeight: '700', marginBottom: '6px' }}>📄 Terms & Conditions</h1>
        ...
        {school.policy_terms ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px', color: 'rgba(255,255,255,0.3)' }}>
            Terms & Conditions not yet published by this school.
          </div>
        )}
