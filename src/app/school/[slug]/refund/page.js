...
export default function SchoolRefundPolicy() {
  ...
  useEffect(() => {
    const fetchSchool = async () => {
      const { data } = await supabase
        .from('schools')
        .select('name, school_address, school_contact_email, slug, primary_color, policy_refund')
        .eq('slug', params.slug)
        .single()
      setSchool(data)
      setLoading(false)
    ...
    <div style={{ minHeight: '100vh', background: '#0f172a', color: '#fff', fontFamily: "'DM Sans', sans-serif", padding: '48px 24px' }}>
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        <div style={{ marginBottom: '40px' }}>
          ...
          <h1 style={{ fontSize: '30px', fontWeight: '700', marginBottom: '6px' }}>💰 Refund & Cancellation Policy</h1>
          <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '13px' }}>Last updated: June 2025</p>
          <div style={{ marginTop: '12px', padding: '12px 16px', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: '8px', color: '#f87171', fontSize: '13px', fontWeight: '600' }}>
            ⚠️ This school operates a No Refund Policy. All fees are non-refundable once enrollment is confirmed.
          </div>
        </div>

        {school.policy_refund ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px', color: 'rgba(255,255,255,0.3)' }}>
            Refund policy not yet published by this school.
          </div>
        )}
