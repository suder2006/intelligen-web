...
export default function TeacherProgressPage() {
  ...
  useEffect(() => { if (selectedProgram) fetchSkillsAndStudents() }, [selectedProgram, selectedTerm, selectedAY])
  useEffect(() => { if (selectedStudent) fetchProgress() }, [selectedStudent, selectedTerm, selectedAY])
  ...
  const fetchSkillsAndStudents = async () => {
    setLoadingSkills(true)
    setSelectedStudent(null)
    setRatings({})
    setReportNotes({ observations: '', strengths: '', areas_to_improve: '' })

    // Get skills assigned to this program + term
    const { data: maps } = await supabase.from('skill_program_map')
      .select('skill_id')
  ...
  const fetchProgress = async () => {
    ...
    const [ratingsRes, reportRes] = await Promise.all([
      supabase.from('progress_ratings').select('*')
    ...
    const map = {}
    ;(ratingsRes.data || []).forEach(r => { map[r.activity_id] = r.rating })
    setRatings(map)
    if (reportRes.data) {
      setReportNotes({ observations: reportRes.data.observations || '', strengths: reportRes.data.strengths || '', areas_to_improve: reportRes.data.areas_to_improve || '' })
      setReportId(reportRes.data.id)
      setAlreadySent(reportRes.data.sent_to_parent || false)
    } else {
      setReportNotes({ observations: '', strengths: '', areas_to_improve: '' })
      setReportId(null)
  ...
  const saveProgress = async () => {
    ...
    for (const [activityId, rating] of Object.entries(ratings)) {
      ...
      await supabase.from('progress_ratings').upsert({
        student_id: selectedStudent.id, activity_id: activityId,
        teacher_id: user.id, academic_year: selectedAY, term: selectedTerm,
        rating, updated_at: new Date().toISOString()
      }, { onConflict: 'student_id,activity_id,academic_year,term' })
    }

    const reportData = { student_id: selectedStudent.id, teacher_id: user.id, academic_year: selectedAY, term: selectedTerm, ...reportNotes, updated_at: new Date().toISOString() }
    if (reportId) {
      await supabase.from('progress_reports').update(reportData).eq('id', reportId)
    } else {
      const { data: nr } = await supabase.from('progress_reports').insert({ ...reportData, sent_to_parent: false }).select().single()
      if (nr) setReportId(nr.id)
    }
    setSaving(false)
    setSaved(true)
  ...
  const sendToParent = async () => {
    if (!selectedStudent) return
    if (!confirm(`Send ${selectedTerm} progress report for ${selectedStudent.full_name} to parents?`)) return
    setSending(true)
    await saveProgress()
    if (reportId) {
    ...
    for (const { parent_id } of (ps || [])) {
      await supabase.from('chat_messages').insert({
        sender_id: profile?.id, receiver_id: parent_id,
        sender_name: profile?.full_name || 'Teacher',
        content: `📊 Progress Report for ${selectedStudent.full_name} — ${selectedTerm} (${selectedAY}) is now available. Please check the Progress tab in your portal.`
      })
    }
    setAlreadySent(true)
    setSending(false)
  ...
  const totalActivities = skills.reduce((s, sk) => s + (sk.skill_activities?.length || 0), 0)
    ...
    <div style={{ minHeight: '100vh', background: '#0f172a', fontFamily: "'DM Sans', sans-serif", color: '#fff' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .term-btn { padding: 8px 20px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); cursor: pointer; font-size: 13px; font-weight: 600; font-family: 'DM Sans', sans-serif; transition: all 0.2s; }
        ...
        .activity-row { display: flex; justify-content: space-between; align-items: center; padding: 12px 0; border-bottom: 1px solid rgba(255,255,255,0.05); gap: 12px; flex-wrap: wrap; }
      ...
      <div className="content">
        <div style={{ marginBottom: '20px' }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '14px' }}>Rate activities per term and send reports to parents</p>
        </div>
        ...
        <div className="card">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '16px', marginBottom: '20px' }}>
            <div>
              ...
              <select value={selectedProgram} onChange={e => setSelectedProgram(e.target.value)}
                style={{ width: '100%', padding: '10px 14px', backgroundColor: '#0f172a', color: '#fff', border: '1px solid #334155', borderRadius: '8px', fontSize: '14px' }}>
            ...
            <div>
              ...
              <input value={selectedAY} onChange={e => setSelectedAY(e.target.value)}
                style={{ width: '100%', padding: '10px 14px', backgroundColor: '#0f172a', color: '#fff', border: '1px solid #334155', borderRadius: '8px', fontSize: '14px' }} />
            </div>
          </div>
          ...
          {/* Student summary bar */}
          {selectedStudent && (
            <div style={{ marginTop: '16px', padding: '12px 16px', background: 'rgba(255,255,255,0.03)', borderRadius: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'linear-gradient(135deg, #0ea5e9, #38bdf8)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: '700' }}>{selectedStudent.full_name?.[0]}</div>
                <div>
                  <div style={{ fontWeight: '700' }}>{selectedStudent.full_name}</div>
                  <div style={{ color: '#a78bfa', fontSize: '12px' }}>{selectedProgram} · {selectedTerm} · {selectedAY}</div>
                </div>
              </div>
              <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ color: '#38bdf8', fontWeight: '700', fontSize: '16px' }}>{ratedCount}/{totalActivities}</div>
                  <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: '11px' }}>Rated</div>
                </div>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ color: '#10b981', fontWeight: '700', fontSize: '16px' }}>{achievedCount}</div>
                  <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: '11px' }}>Achieved</div>
                </div>
                {alreadySent && <span style={{ padding: '3px 10px', background: 'rgba(16,185,129,0.15)', color: '#34d399', borderRadius: '20px', fontSize: '12px', fontWeight: '600' }}>✅ Sent to Parent</span>}
              </div>
            </div>
          )}
        </div>
        ...
        {loadingSkills ? (
          <div style={{ textAlign: 'center', padding: '40px', color: 'rgba(255,255,255,0.3)' }}>Loading skills...</div>
        ) : selectedStudent && skills.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px', background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.15)', borderRadius: '16px', color: '#f59e0b' }}>
            ⚠️ No skills assigned to <strong>{selectedProgram}</strong> for <strong>{selectedTerm}</strong>.<br/>
            <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: '13px' }}>Ask admin to assign skills in Skills & Progress setup.</span>
          </div>
        ) : selectedStudent && (
          <>
            {/* Rating Legend */}
            <div style={{ display: 'flex', gap: '10px', marginBottom: '16px', flexWrap: 'wrap' }}>
              {RATINGS.map(r => (
                <span key={r.value} style={{ padding: '5px 14px', borderRadius: '20px', fontSize: '13px', fontWeight: '600', background: r.bg, color: r.color, border: `1px solid ${r.border}` }}>{r.label}</span>
              ))}
            </div>
                  ...
                  return (
                    <div key={act.id} className="activity-row">
                      ...
                      <div style={{ display: 'flex', gap: '6px' }}>
                        {RATINGS.map(r => (
                          <button key={r.value} className="rating-btn" onClick={() => setRating(act.id, r.value)}
                            style={{ background: curr === r.value ? r.bg : 'transparent', border: `1px solid ${curr === r.value ? r.border : 'rgba(255,255,255,0.1)'}`, color: curr === r.value ? r.color : 'rgba(255,255,255,0.3)' }}>
                            {r.short}
                          </button>
                        ))}
                      </div>
            ...
            <div className="card">
              ...
              <textarea rows={3} value={reportNotes.observations} onChange={e => setReportNotes({ ...reportNotes, observations: e.target.value })}
                placeholder='Overall behavior and learning this term...' style={{ ...inputStyle, marginBottom: '14px' }} />
              ...
              <textarea rows={2} value={reportNotes.strengths} onChange={e => setReportNotes({ ...reportNotes, strengths: e.target.value })}
                placeholder='What is the child doing well?' style={{ ...inputStyle, marginBottom: '14px' }} />
              ...
              <textarea rows={2} value={reportNotes.areas_to_improve} onChange={e => setReportNotes({ ...reportNotes, areas_to_improve: e.target.value })}
                placeholder='What needs more focus?' style={{ ...inputStyle, marginBottom: '20px' }} />
              ...
              <div style={{ display: 'flex', gap: '12px' }}>
                ...
                <button onClick={sendToParent} disabled={sending || alreadySent}
                  style={{ flex: 1, padding: '12px', background: alreadySent ? 'rgba(16,185,129,0.08)' : sent ? '#10b981' : 'linear-gradient(135deg, #0ea5e9, #38bdf8)', border: alreadySent ? '1px solid rgba(16,185,129,0.3)' : 'none', borderRadius: '10px', color: alreadySent ? '#34d399' : '#fff', fontWeight: '700', fontSize: '14px', cursor: alreadySent ? 'not-allowed' : 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
                  {sending ? '⏳ Sending...' : sent ? '✅ Sent!' : alreadySent ? '✅ Already Sent' : '📤 Send to Parent'}
                </button>
              </div>
            </div>
          </>
        ...
        {!selectedStudent && !loading && !loadingSkills && (
          <div style={{ textAlign: 'center', padding: '60px', color: 'rgba(255,255,255,0.3)' }}>
            <div style={{ fontSize: '48px', marginBottom: '16px' }}>📊</div>
            <div>Select a program, term and student to start</div>
          </div>
        )}
      </div>
    </div>
