...
export default function StudentsPage() {
  ...
  async function archiveStudent() {
    ...
    const archivedName = archiveModal.full_name
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#0f172a', padding: '24px', color: '#fff' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '32px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <h1 style={{ fontSize: '28px', fontWeight: 'bold' }}>
              {showArchived ? '🗃️ Archived Students' : '👶 Students'} ({students.length})
            </h1>
          </div>
        ...
        {showForm && !showArchived && (
          <div style={{ backgroundColor: '#1e293b', borderRadius: '16px', padding: '24px', border: '1px solid #38bdf8', marginBottom: '24px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px', marginBottom: '16px' }}>
              ...
              <div><label style={{ color: '#94a3b8', fontSize: '13px' }}>Date of Birth</label><input type='date' value={form.date_of_birth} onChange={e => setForm({ ...form, date_of_birth: e.target.value })} style={inputStyle} /></div>
              <div><label style={{ color: '#94a3b8', fontSize: '13px' }}>Gender</label><select value={form.gender} onChange={e => setForm({ ...form, gender: e.target.value })} style={inputStyle}><option value=''>-- Select --</option><option value='Male'>Male</option><option value='Female'>Female</option></select></div>
              <div><label style={{ color: '#94a3b8', fontSize: '13px' }}>Program *</label><select value={form.program} onChange={e => setForm({ ...form, program: e.target.value })} style={inputStyle}><option value=''>-- Select Program --</option>{programs.map(p => <option key={p} value={p}>{p}</option>)}</select></div>
              <div><label style={{ color: '#94a3b8', fontSize: '13px' }}>Status</label><select value={form.status} onChange={e => setForm({ ...form, status: e.target.value })} style={inputStyle}><option value='active'>Active</option><option value='inactive'>Inactive</option></select></div>
              ...
              <div><label style={{ color: '#94a3b8', fontSize: '13px' }}>Emergency Contact</label><input value={form.emergency_contact} onChange={e => setForm({ ...form, emergency_contact: e.target.value })} placeholder='+91 98765 43210' style={inputStyle} /></div>
              <div><label style={{ color: '#94a3b8', fontSize: '13px' }}>Authorized Pickup Persons</label><input value={form.authorized_pickup} onChange={e => setForm({ ...form, authorized_pickup: e.target.value })} placeholder='e.g. Father, Mother, Grandparent' style={inputStyle} /></div>
              <div><label style={{ color: '#94a3b8', fontSize: '13px' }}>Allergies</label><input value={form.allergies} onChange={e => setForm({ ...form, allergies: e.target.value })} placeholder='e.g. Nuts, Dairy, Gluten' style={inputStyle} /></div>
              <div><label style={{ color: '#94a3b8', fontSize: '13px' }}>Medical Info / Conditions</label><input value={form.medical_info} onChange={e => setForm({ ...form, medical_info: e.target.value })} placeholder='e.g. Asthma, Diabetes' style={inputStyle} /></div>
              <div><label style={{ color: '#94a3b8', fontSize: '13px' }}>Medical Alert Note</label><input value={form.medical_alert_note} onChange={e => setForm({ ...form, medical_alert_note: e.target.value })} placeholder='e.g. Carry EpiPen at all times' style={inputStyle} /></div>
              <div><label style={{ color: '#94a3b8', fontSize: '13px' }}>Child Aadhar Number</label><input value={form.child_aadhar} onChange={e => setForm({ ...form, child_aadhar: e.target.value })} placeholder='12 digit Aadhar number' style={inputStyle} /></div>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={save} disabled={saving} style={{ padding: '10px 24px', backgroundColor: '#38bdf8', color: '#0f172a', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>
                {saving ? 'Saving...' : editing ? '✏️ Update' : '💾 Save Student'}
              </button>
              <button onClick={() => { setShowForm(false); setEditing(null) }} style={{ padding: '10px 24px', backgroundColor: '#334155', color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>Cancel</button>
            </div>
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
          <input placeholder={showArchived ? 'Search archived students...' : 'Search students...'} value={search} onChange={e => setSearch(e.target.value)}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #334155' }}>
                {['Name', 'Program', 'ID', 'DOB', 'Gender', 'Parent', 'Phone', showArchived ? 'Archive Reason' : 'Status', ''].map(h => (
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={9} style={{ textAlign: 'center', padding: '40px', color: '#475569' }}>Loading...</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={9} style={{ textAlign: 'center', padding: '40px', color: '#475569' }}>
                ...
                <tr key={s.id} style={{ borderBottom: '1px solid #1e293b' }}>
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    {showArchived ? (
                      <div>
                        <span style={{ backgroundColor: 'rgba(239,68,68,0.15)', color: '#f87171', padding: '3px 10px', borderRadius: '20px', fontSize: '12px' }}>{s.archive_reason || '—'}</span>
                        {s.archived_at && <div style={{ color: '#475569', fontSize: '11px', marginTop: '4px' }}>{new Date(s.archived_at).toLocaleDateString()}</div>}
                      </div>
                    ) : (
                    ...
                    <div style={{ display: 'flex', gap: '6px' }}>
                      {showArchived ? (
                        <button onClick={() => restoreStudent(s)}
                          style={{ padding: '5px 12px', backgroundColor: 'rgba(16,185,129,0.15)', color: '#34d399', border: '1px solid rgba(16,185,129,0.3)', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', fontWeight: '600' }}>
                          ♻️ Restore
                        </button>
                      ) : (
                        <>
