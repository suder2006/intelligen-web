...
export default function CheckinPage() {
  ...
  const [result, setResult] = useState(null) // { type: 'success'|'error', message, action }
  const [scanMode, setScanMode] = useState('staff') // staff | student
  ...
  const init = async () => {
    ...
    if (user) {
      ...
      setProfile(prof)
      // If token present in URL, auto process staff check-in
      if (token === SCHOOL_TOKEN && prof?.role !== 'parent') {
        await processStaffCheckin(user, prof)
      }
    }
    // Load students for manual check-in
  ...
  const processStaffCheckin = async (u, prof) => {
    ...
    if (!existing) {
      ...
      if (grpData) {
        ...
        else if (timeStr < grpData.checkin_start) status = 'early'
      }
  ...
  const processStudentCheckin = async (student, method = 'qr') => {
    setProcessing(true)
    const today = new Date().toISOString().split('T')[0]
    const now = new Date()
    ...
    if (!existing) {
      ...
      await supabase.from('student_checkins').insert({
        ...
        checkin_time: now.toISOString(),
        checkin_by: u?.id, checkin_by_name: profile?.full_name || 'Gate',
      ...
      await supabase.from('student_checkins').update({
        checkout_time: now.toISOString(),
        checkout_by: u?.id, checkout_by_name: profile?.full_name || 'Gate',
  ...
  // Handle QR scan redirect for student (URL: /checkin?student=STUDENT_ID)
  const studentId = searchParams.get('student')
  useEffect(() => {
    if (studentId && students.length > 0) {
      const student = students.find(s => s.id === studentId)
      if (student) processStudentCheckin(student, 'qr')
    }
  }, [studentId, students])
          ...
          <>
            ...
            {result && (
              <div style={{ background: result.type === 'success' ? 'rgba(16,185,129,0.15)' : 'rgba(56,189,248,0.1)', border: `1px solid ${result.type === 'success' ? 'rgba(16,185,129,0.3)' : 'rgba(56,189,248,0.2)'}`, borderRadius: '16px', padding: '24px', textAlign: 'center', marginBottom: '24px' }}>
                ...
                {result.status && <div style={{ padding: '4px 14px', borderRadius: '20px', fontSize: '13px', fontWeight: '600', display: 'inline-block', background: `${statusColor[result.status]}22`, color: statusColor[result.status] }}>{result.status}</div>}
            ...
            {!result && (
              <>
                {/* Staff Check-in (if logged in as staff/teacher) */}
                ...
                <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '16px', padding: '24px' }}>
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    {searchResults.length > 0 && (
                      <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: '#1e293b', border: '1px solid #334155', borderRadius: '10px', marginTop: '4px', zIndex: 10, overflow: 'hidden' }}>
                        {searchResults.map(s => (
                          <button key={s.id} onClick={() => processStudentCheckin(s, 'manual')} disabled={processing}
                  ...
                  <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: '12px', marginTop: '10px', textAlign: 'center' }}>
                    Type student name to search and check in/out manually
                  </div>
                </div>
